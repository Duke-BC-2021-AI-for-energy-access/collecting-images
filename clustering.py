...
def apply_clustering(input_filepath: Path,
    ...
    Arguments:
     ...
     region: geographical area must be one of [NE, NW, EM, SW, MW] or None to apply
    ...
    if region in ['NE', 'NW', 'EM', 'SW', 'MW']:
        ...
        print(f'{region} is an invalid region [NE, NW, EM, SW, MW]')
        return
...
def stratified_split(df: pd.DataFrame,
    ...
    Arguments:
     df: input dataframe
     output_filepath: directory path to store stratified train and test data
    ...
    train_set.to_csv(os.path.join(output_filepath, f'{region}_train_coord.csv'), index=False)
    test_set.to_csv(os.path.join(output_filepath, f'{region}_test_coord.csv'), index=False)
